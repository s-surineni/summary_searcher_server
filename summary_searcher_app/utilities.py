...
def search_summaries(search_text):
    ...
    for word in search_text_sanitized_unique:
        word_obj = InvertedIndex.objects.filter(word=word)
        if not word_obj:
            continue
        word_obj = word_obj[0]
        summaries_containing_words = [summary_id for summary_id
                                      in json.loads(word_obj.summaries).keys()]
    ...
    max_word_summaries = []
    for a_summary in freq_summary_count[::-1]:
        max_word_summaries.append({'summary_id': a_summary[1],
                                   'words_matched': summary_words_map[a_summary[1]]})
